from Transaction import addCommissionedEmployeetransaction, addHourlyEmployeetransaction, addSalariedEmployeetransaction
...
class parser():
    ...
    def parseTransaction(self, transactionType):
        ...
        if(transactionType == "AddSalariedEmployee"):
            transaction = addSalariedEmployeetransaction()
        if(transactionType == "AddSaleReceipt"):
            transaction = addCommissionedEmployeetransaction()
        if(transactionType == "AddCommissionedEmployee"):
            transaction = addCommissionedEmployeetransaction()
        if(transactionType == "AddCommissionedEmployee"):
