...
from DataStore import dataStore, employeeList
...
class addEmployeeTransaction(transaction):
    ...
    _paymentClassification: payClassification
    ...
    def addToStore(self):
        emp = employee(self.getName(), self.getAddress(), self._paymentClassification, 
                self.getPaymentMethod(), self.getAffliation(), self.getSchedule())
        dataStore.addEmployee(self, emp)
        return dataStore.getEmployeeCount(self)
